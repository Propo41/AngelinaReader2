...
class Engine(object):
    ...
    def run(self, data, max_epochs=1):
        ...
        if self.loaded_state:
            self.state.iteration = self.loaded_state.get("iteration", self.state.iteration)
            self.state.epoch = self.loaded_state.get("epoch", self.state.epoch)
            self.state.metrics = self.loaded_state.get("metrics", self.state.metrics)
            self.loaded_state = None
